#!/bin/sh -x
#
# Build script for MS Office 2019 Standard for Mac for Volume Licensing Distribution
#
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
do
	...
	if [ $? -eq 0 ]; then
		...
		echo "./MS_Office_2019_PKGBuilder.sh --exclude \"onenote.mac OneDrive autoupdate\""
		exit 1
	fi
done
...
cp -rp "${LICENSE_DIR}"/${Serializer} "${SCRIPT_DIR}"
...
curl -LsJ $FULL_INSTALLER -o "${SCRIPT_DIR}"/Microsoft_Office_2019_Installer.pkg

# generate postinstall script
cat <<- 'EOF' > "${TEMP_DIR}"/postinstall
#!/bin/bash
...
# Define what Apps are part of Office 2019, if anything changes in the future
Office2019Apps=(Excel OneDrive OneNote Outlook PowerPoint Word)

# Determine working directory
WORKING_DIR=`dirname $0`

# Set Log File
LOGFILE="/var/log/MyLogs/MS_Office_2019.log"
...
ConfigureOffice2019FirstRun()
{
    # This function will configure the first run dialog windows for all Office 2019 apps.
...
# Remove an old volume licensing file
# Any old version for VL2016 will not work with VL2019
...
if [[ -e "$WORKING_DIR/choices.xml" ]]; then
    ...
    /usr/sbin/installer -dumplog -verbose -pkg "$WORKING_DIR/Microsoft_Office_2019_Installer.pkg" -applyChoiceChangesXML "$WORKING_DIR/choices.xml" -target "$3"   2>&1 | tee -a $LOGFILE
else
    echo "InstallerChoices file not found! Continuing!"      2>&1 | tee -a $LOGFILE
    /usr/sbin/installer -dumplog -verbose -pkg "$WORKING_DIR/Microsoft_Office_2019_Installer.pkg" -target "$3"   2>&1 | tee -a $LOGFILE
fi

# Install the Microsoft Office 2016 Volume License file from
/usr/sbin/installer -dumplog -verbose -pkg "$WORKING_DIR/PLACEHOLDER" -target "$3"   2>&1 | tee -a $LOGFILE
...
sed -e "s/PLACEHOLDER/${Serializer}/" "${TEMP_DIR}"/postinstall > "${SCRIPT_DIR}"/postinstall
